...
# Sparse Sequence Data Generator Runner - Dynamic for Local/SLURM environments
# Generates sparse event format for continuous DST training
...
if [ "$IS_SLURM" = true ]; then
    ...
    d_folder=$(date +'%Y-%m-%d')
    SLURM_FOLDER_BASE=slurm_out/sparse_gen/
...
run_generator() {
    echo "╔══════════════════════════════════════════════╗"
    echo "║   Sparse Sequence Generator${IS_SLURM:+ (SLURM)}   ║"
    echo "╚══════════════════════════════════════════════╝"
    ...
    # Run the sparse sequence generator module
    python -m dst_data_builder.sparse_sequence_generator

    echo ""
    echo "✅ Sparse sequence data generation completed successfully!"
}
