        ...
        **kwargs,
        ...
        if output_hidden_states and outputs.hidden_states:
            ...
            if self.dst_update_head:
                outputs.dst_update_logits = self.dst_update_head(last_hidden).squeeze(
                    -1
                )
        ...
        # Shift efficiently: logits[:-1] predicts labels[1:]
        if speaking_gen_labels is not None:
            # Memory-efficient shift: slice without creating full copies
            # Use views instead of .contiguous() to save memory
            mask = speaking_gen_labels[..., 1:] != -100
            if mask.any():
