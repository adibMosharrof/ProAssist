...
class ProspectEvaluator:
    ...
    def _print_results(self, metrics: dict):
        ...
        # NLG metrics
        if "dialog_Bleu_4" in metrics:
            self.logger.info(f"\n📝 NLG Quality Metrics:")
            for i in range(1, 5):
                key = f"dialog_Bleu_{i}"
                if key in metrics:
                    self.logger.info(f"  BLEU-{i}:   {metrics[key]:.4f}")
            if "dialog_CIDEr" in metrics:
                self.logger.info(f"  CIDEr:    {metrics['dialog_CIDEr']:.4f}")
            if "dialog_METEOR" in metrics:
                self.logger.info(f"  METEOR:   {metrics['dialog_METEOR']:.4f}")
        ...
        self.logger.info(f"  PROSPECT (baseline): F1 {metrics.get('F1', 0):.4f}, BLEU-4 {metrics.get('dialog_Bleu_4', 0):.4f}")
