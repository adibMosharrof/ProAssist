...
class DSTDataProcessor:
    ...
    def process_single_video(self, video_data: dict, video_index: int, dataset_name: str) -> tuple[list, int]:
        """
        ...
        for conv_idx, conversation in enumerate(conversations):
            ...
            if self.speak_dst_generator:
                conversation_example = self._apply_speak_dst_transformation(conversation_example)
    ...
    def process_dataset_split(
        ...
        video_data_pairs = [(video_data, video_index, dataset_name) for video_index, video_data in enumerate(limited_data)]
        
        if self.is_multiprocessing:
            # Get generator configs for worker processes
            dst_config = self._extract_dst_generator_config()
            speak_config = self._extract_speak_dst_generator_config() if self.speak_dst_generator else None
            
            # Extend video_data_pairs with configs for each worker call
            extended_data_pairs = [(video_data, video_index, dataset_name, dst_config, speak_config)
                                 for video_data, video_index, dataset_name in video_data_pairs]
            
            # Parallel processing using ProcessPoolExecutor
            self.logger.info(f"🚀 Starting parallel processing with {len(limited_data)} videos using {self.num_processes} processes")
            
            with tqdm(total=len(limited_data), desc=f"🎬 {dataset_name}/{split} videos",
                     unit="video") as video_pbar:

                with ProcessPoolExecutor(max_workers=self.num_processes) as executor:
                    results = []
                    for result in executor.map(_unpack_and_process_video_worker, extended_data_pairs):
    ...
    def _load_and_limit_data(self, input_file: Path, num_rows: int) -> List[Dict[str, Any]]:
    ...
    def _generate_dst_labels(self, video_data: dict, video_index: int, dataset_name: str) -> dict:
    ...
    def _extract_all_conversation_descriptions(self, video_data: dict) -> str:
        ...
        if conversations and len(conversations) > 0:
            ...
            for conv_idx, conversation in enumerate(conversations):
                ...
                conv_user_type = conversation.get('user_type', f'conversation_{conv_idx}')
                ...
                for item in conversation_items:
                    ...
                    if role and content:
                        dialog_content.append(f"[{time:.1f}] {role}: {content} (from {conv_user_type})")
            all_step_descriptions = "\n".join(dialog_content)
    ...
    def _apply_speak_dst_transformation(self, conversation_example: dict) -> dict:
        ...
        try:
            # Transform for SPEAK/DST format
            enhanced_data = {
                'conversations': [{
                    'conversation': conversation_example['conversation'],
                    'user_type': conversation_example['metadata']['user_type'] + '_dst_enhanced'
                }],
                'dst': conversation_example['dst']
            }
            
            enhanced_data = self.speak_dst_generator.transform_video_data(enhanced_data)
            
            # Update the conversation example with enhanced format
            conversation_example['conversation'] = enhanced_data['conversations'][0]['conversation']
            conversation_example['metadata']['user_type'] = enhanced_data['conversations'][0]['user_type']
            
            return conversation_example
            
        except Exception as e:
            self.logger.warning(f"Failed to apply SPEAK/DST transformation: {e}. Using original format.")
            return conversation_example

    def _save_processed_data(self, processed_conversations: List[dict], output_file: Path) -> None:
        """Save processed conversations to output file"""
        
        with open(output_file, 'w') as f:
            json.dump(processed_conversations, f, indent=2)
        
        self.logger.debug(f"💾 Saved {len(processed_conversations)} conversations to {output_file}")

    def _extract_dst_generator_config(self) -> Dict[str, Any]:
        """Extract config needed to recreate dst generator for worker processes"""
        return {
            'generator_type': getattr(self.dst_generator, 'generator_type', 'proassist_label'),
    ...
    def _extract_speak_dst_generator_config(self) -> Optional[Dict[str, Any]]:
        ...
        return {
            'cfg': self.speak_dst_generator.cfg
        }


def _process_video_worker(video_data: dict, video_index: int, dataset_name: str,
                         dst_generator_config: Dict[str, Any],
                         speak_dst_config: Optional[Dict[str, Any]] = None) -> tuple[list, int]:
    """
...
def _cleanup_worker_memory(processor: DSTDataProcessor):
    ...
    if hasattr(processor, 'dst_generator'):
        ...
        if hasattr(processor.dst_generator, 'encoder') and processor.dst_generator.encoder is not None:
            del processor.dst_generator.encoder
        if hasattr(processor.dst_generator, 'nli_model') and processor.dst_generator.nli_model is not None:
