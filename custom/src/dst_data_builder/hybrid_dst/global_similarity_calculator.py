...
class GlobalSimilarityCalculator:
    ...
    def __init__(self, config: DictConfig):
        ...
        # Configuration
        self.semantic_weight = config.get("semantic_similarity_weight", 0.6)
        self.nli_weight = config.get("nli_score_weight", 0.4)
        self.high_confidence_threshold = config.get("high_confidence_threshold", 0.3)
    ...
    def _calculate_confidence(self, scores: np.ndarray) -> float:
        ...
        if len(scores) == 1:
            return 0, 1.0  # Only one option, fully confident
