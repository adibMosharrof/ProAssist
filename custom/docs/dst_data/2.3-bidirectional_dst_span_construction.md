# 🔄 Bidirectional DST Span Construction Algorithm
...
3. **Conflict Resolution**: Where forward/backward disagree = ambiguous → LLM
4. **Temporal Anchoring**: First block always → first step, Last block always → last step
...
**Output**: List of assignments: `[(block_0 → step_0), (block_1 → step_1), ...]`
...
for each block_idx:
    if forward[block_idx].step != backward[block_idx].step:
        ...
        # Low confidence even if they agree → conflict
        mark as conflict
```
...
2. Forward pass with high confidence (≥ 0.3)
3. Backward pass with high confidence (≥ 0.3)
...
```
Block 0 → Step 0 (confidence: 1.0, anchored)
Block 1 → Step 1 (confidence: 0.85, high similarity to "attach wheels")
Block 2 → Step 2 (confidence: 0.45, transition to "assemble arm")
Block 3 → Step 2 (confidence: 0.55, "attach hook" related to arm assembly)
Block 4 → Step 2 (confidence: 0.60, "turntable to chassis" part of arm assembly)
Block 5 → Step 4 (confidence: 0.70, "cabin" matches "body")
Block 6 → Step 5 (confidence: 1.0, anchored to last step)
```
...
```
Block 6 → Step 5 (confidence: 1.0, anchored)
Block 5 → Step 4 (confidence: 0.75, "cabin" matches step 4)
Block 4 → Step 3 (confidence: 0.28, LOW - "turntable" ambiguous)
Block 3 → Step 2 (confidence: 0.50, "hook to arm" matches step 2)
Block 2 → Step 2 (confidence: 0.62, "arm to turntable" matches step 2)
Block 1 → Step 1 (confidence: 0.88, "wheel" clearly matches)
Block 0 → Step 0 (confidence: 1.0, anchored)
```
...
**Resolution**: Block 4 → Step 2 (0-indexed)
...
```
Block 0 → Step 0 (from forward, anchored)
Block 1 → Step 1 (from forward, high confidence)
Block 2 → Step 2 (from forward, medium confidence)
Block 3 → Step 2 (from forward, medium confidence)
Block 4 → Step 2 (from LLM resolution)
Block 5 → Step 3 (from backward, high confidence)
Block 6 → Step 5 (from backward, anchored)
```
