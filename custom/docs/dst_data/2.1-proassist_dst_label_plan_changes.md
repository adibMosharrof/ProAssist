# 📘 DST Generation Pipeline with Overlap-Aware Block Reduction
...
| 2     | DST Span Construction          | Align filtered blocks to inferred knowledge with boundary detection |
...
## 🧩 Stage 1: Overlap-Aware Block Reduction
...
- Remove any blocks that fall completely within these main block ranges:
  - **Interval blocks**: Remove blocks with time ranges contained within main block ranges
  - **Point blocks**: Remove blocks with single timestamps that fall within main block ranges
- Keep only the top-level, non-overlapping blocks for DST alignment
- Point blocks use their single timestamp as start time and are merged if within interval blocks


### 🔍 Redundancy Detection Criteria

1. **Time Containment for Interval Blocks**
   Check if one interval block's time range is completely contained within another interval block's range

   ```
   Interval Block B is contained in Interval Block A if:
   A.t0 ≤ B.t0 and B.t1 ≤ A.t1
   ```

   - If Interval Block B falls completely within Interval Block A's time range: merge B into A
   - No semantic similarity or verb/object matching required

2. **Point Block Containment**
   Handle blocks with only 1 timestamp (point blocks)

   ```
   Point Block P has only: P.t0 (start time, end_time = None)
   Point Block P is contained in Interval Block A if:
   A.t0 ≤ P.t0 ≤ A.t1
   ```

   - Point blocks are assigned the single timestamp as their start time
   - If the point block's timestamp falls within the interval block's range: merge P into A
   - Point blocks cannot contain other blocks (no end time)
   - Point blocks are merged into containing interval blocks
...
- Find all other blocks that fall completely within this main block's time range:
  ...
  **Interval Block Merging:**
  - Interval blocks fully contained within main block → merge into main block
  
  **Point Block Merging:**
  - Point blocks (single timestamp) whose timestamp falls within main block's range → merge into main block
...
1. 🟢 [94.4s-105.2s] attach interior to chassis
2. 🔵 [105.2s-153.6s] attach wheel to chassis
3. 🔵 [111.0s] screw first wheel with hand
4. 🔵 [115.7s] screw second wheel with hand
5. 🔵 [116.2s] screw second wheel with hand
6. 🔵 [117.1s] screw first wheel with screwdriver
7. 🔵 [117.3s] screw first wheel with screwdriver
8. 🔵 [120.0s] screw second wheel with screwdriver
9. 🔵 [127.7s] screw third wheel with screwdriver
10. 🔵 [138.8s] screw fourth wheel with screwdriver
11. 🔵 [141.0s] screw fourth wheel with screwdriver
12. 🟡 [153.6s-171.7s] attach arm to turntable top
13. 🟡 [164.2s] screw turntable top with screwdriver
14. 🟡 [169.5s] screw turntable top with hand
15. ⚪ [171.7s-187.1s] attach hook to arm
16. ⚪ [180.5s] screw hook with hand
17. ⚪ [183.6s] screw hook with hand
18. 🔴 [187.1s-203.7s] attach turntable top to chassis
19. 🟣 [203.7s-213.1s] attach cabin to interior
20. ⚫ [213.1s-232.0s] demonstrate functionality

**Output: `filtered_blocks` (after Stage 1)**
1. 🟢 [94.4s-105.2s] attach interior to chassis
2. 🔵 [105.2s-153.6s] attach wheel to chassis ← merged items 3-11 (9 contained blocks)
3. 🟡 [153.6s-171.7s] attach arm to turntable top ← merged items 13-14 (2 contained blocks)
4. ⚪ [171.7s-187.1s] attach hook to arm ← merged items 16-17 (2 contained blocks)
5. 🔴 [187.1s-203.7s] attach turntable top to chassis
6. 🟣 [203.7s-213.1s] attach cabin to interior
7. ⚫ [213.1s-232.0s] demonstrate functionality

**Explanation:**
- Point actions (111.0s, 115.7s, 116.2s, 117.1s, 117.3s, 120.0s, 127.7s, 138.8s, 141.0s) 
  - Single timestamps: `[111.0s]`, `[115.7s]`, etc. (end_time = None)
  - All fell within "attach wheel to chassis" interval (105.2s-153.6s) → merged
- Point actions (164.2s, 169.5s) were contained within "attach arm to turntable top" block (153.6s-171.7s) → merged
- Point actions (180.5s, 183.6s) were contained within "attach hook to arm" block (171.7s-187.1s) → merged
- All main blocks had no overlapping time ranges → all kept as top-level blocks
- Point blocks used their single timestamp as start time, merged if falling within interval blocks
...
| Condition | Constructor | Approach |
|-----------|-------------|----------|
| `len(filtered_blocks) == len(inferred_knowledge)` | **Simple Span** | Direct timestamp assignment |
| `len(filtered_blocks) != len(inferred_knowledge)` | **Hybrid Span** | Hybrid boundary detection with similarity scoring + LLM fallback |

---

## 🔄 Stage 2A: Simple Span Constructor - Direct Timestamp Assignment

### 🎯 Goal
When the number of filtered blocks exactly matches the number of inferred knowledge steps, use direct timestamp assignment for optimal efficiency and accuracy.

### 📥 Inputs
- `filtered_blocks` (after Stage 1)
- `inferred_knowledge` (high-level steps)
- **Condition**: `len(filtered_blocks) == len(inferred_knowledge)`

### ⚙️ Simple Algorithm

**Direct Assignment Logic:**
```python
def simple_span_constructor(filtered_blocks, inferred_knowledge):
    """
    ...
    assert len(filtered_blocks) == len(inferred_knowledge), \
        "Plan A requires equal number of blocks and steps"
...
1. [94.4s-105.2s] attach interior to chassis
2. [105.2s-153.6s] attach wheel to chassis  
3. [153.6s-171.7s] attach arm to turntable top
4. [171.7s-187.1s] attach hook to arm
5. [187.1s-203.7s] attach turntable top to chassis
6. [203.7s-213.1s] attach cabin to interior
7. [213.1s-232.0s] demonstrate functionality

Inferred Knowledge (7 steps):
1. Assemble the chassis by attaching and screwing the chassis parts together.
2. Attach wheels to the chassis.
3. Assemble the arm and attach it to the chassis.
4. Attach the body to the chassis.
...
```
1. [94.4s-105.2s] Assemble the chassis by attaching and screwing the chassis parts together. (conf: 1.0)
2. [105.2s-153.6s] Attach wheels to the chassis. (conf: 1.0)
3. [153.6s-171.7s] Assemble the arm and attach it to the chassis. (conf: 1.0)
4. [171.7s-187.1s] Attach the body to the chassis. (conf: 1.0)
5. [187.1s-203.7s] Add the cabin window to the chassis. (conf: 1.0)
6. [203.7s-213.1s] Finalize the assembly and demonstrate the toy's functionality. (conf: 1.0)
```

### ✅ Benefits
- **Maximum Efficiency**: O(n) time complexity, no AI model calls
- **High Accuracy**: Direct temporal mapping ensures correct boundaries
- **Low Cost**: No LLM queries required
- **Predictable**: Consistent results with full confidence scores

---

## 🔁 Stage 2B: Hybrid Span Constructor - Two-Phase Boundary Detection
...
# Combine scores: α · semantic + (1-α) · NLI
combined_matrix = α · semantic_matrix + (1-α) · nli_matrix
...
- **Scalability**: O(n×m) with better constants than nested loops
...
5. 🟣 [203.7s-213.0s] Add the cabin window to the chassis.
6. 🔴 [213.0s-232.0s] Finalize the assembly and demonstrate the toy's functionality.
...
2. **Correct Ordering** (kb[i].t1 <= kb[i+1].t0):
   - Validation passes
...
| 2A   | **Simple Span Constructor** - Direct assignment | `inferred_knowledge`, `filtered_blocks` (when counts match) | final DST spans |
| 2B   | **Hybrid Span Constructor** - Hybrid boundary detection | `inferred_knowledge`, `filtered_blocks` (complex cases) | final DST spans |
| 3    | Temporal validation | final DST spans | validated DST spans |

### 🔀 Decision Logic
```python
if len(filtered_blocks) == len(inferred_knowledge):
    # Use Simple Span Constructor - Direct timestamp assignment
    final_spans = simple_span_constructor(filtered_blocks, inferred_knowledge)
else:
    # Use Hybrid Span Constructor - Hybrid boundary detection
    final_spans = hybrid_span_constructor(filtered_blocks, inferred_knowledge)
```
...
- 🧹 **Cleaner inputs** → reduces scoring noise  
- 🔁 **Reliable alignment** → based on real text + time  
- 🛠 **Precise corrections** → improves span quality without overfitting  
- 🚫 **No hallucination** → timestamps are grounded in data
