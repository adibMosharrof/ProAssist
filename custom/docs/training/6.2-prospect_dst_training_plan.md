# 🎯 PROSPECT DST Training Implementation Plan
...
**Key Changes:**
- **Replace:** Progress summaries → DST state
...
## 🏗️ Phase 2: Model Architecture Updates
...
class DSTTrainer:
    ...
    - Built-in validation via HuggingFace Trainer
    """
    ...
    def setup_training_args(self):
        ...
        self.training_args = TrainingArguments(
            ...
            # Validation configuration (built into Trainer)
            evaluation_strategy=self.config.eval_strategy,  # "steps"
            eval_steps=self.config.eval_steps,               # 500
    ...
    def setup_data_collator(self):
        """Initialize data collator following ProAssist pattern"""
    ...
    def train(self):
        """
        Main training loop with built-in validation
        
        HuggingFace Trainer automatically:
        - Runs validation every eval_steps (500)
        - Computes eval_loss (combined LM + DST binary + DST generation)
        - Calls compute_metrics if provided
        ...
        trainer = Trainer(
            ...
            compute_metrics=self.compute_metrics,       # Optional: additional metrics
        )
        
        # Train from scratch
        # Validation runs automatically every eval_steps
        trainer.train()
    ...
    def compute_metrics(self, eval_preds):
        """
        Optional metrics callback for validation
        
        Called automatically by Trainer during validation
        Computes additional metrics beyond loss
        """
        predictions, labels = eval_preds
        
        # Extract predictions for each task
        lm_preds = predictions.get('lm_logits')
        dst_binary_preds = predictions.get('dst_binary_logits')
        dst_gen_preds = predictions.get('dst_gen_logits')
        
        metrics = {}
        
        # 1. Perplexity (language modeling)
        if lm_preds is not None:
            metrics['perplexity'] = compute_perplexity(lm_preds, labels['lm_labels'])
        
        # 2. DST binary F1
        if dst_binary_preds is not None:
            metrics['dst_binary_f1'] = compute_binary_f1(
                dst_binary_preds, 
                labels['dst_binary_labels']
            )
        
        # 3. DST generation accuracy
        if dst_gen_preds is not None:
            metrics['dst_gen_accuracy'] = compute_dst_accuracy(
                dst_gen_preds,
...
**Note:** Validation is fully integrated into HuggingFace Trainer. No separate validation loop needed. The Trainer handles:
- Computing validation loss every `eval_steps`
- Calling `compute_metrics` if provided
- Logging all metrics automatically
- Saving checkpoints based on validation performance
...
3. **DST Generation Target - Multiple Updates**
   - Format for single update: `S1->start` ✓
